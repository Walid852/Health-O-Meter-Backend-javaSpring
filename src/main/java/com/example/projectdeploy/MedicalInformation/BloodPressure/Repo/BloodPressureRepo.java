...
public interface BloodPressureRepo extends JpaRepository<BloodPressure,UUID> {
    ...
    @Query("SELECT O.date, AVG(O.systolic) AS average_systolic, AVG(O.diastolic) AS diastolic  FROM BloodPressure O where O.medicalInformation.id=?1 and O.isDeleted=false and O.date between ?2 and ?3 GROUP BY O.date order by O.date asc")
